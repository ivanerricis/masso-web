        ...
        {panel.secretsToReconfigure.length > 0 ? (
          <div className="grid gap-1 rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-sm">
            <p className="font-medium">
              Password da reinserire dopo il ripristino
            </p>
            <p className="text-muted-foreground">
              Le password sono cifrate con una chiave che resta sul server e non
              viene inclusa nei backup. Ripristinando su un&apos;altra macchina
              non sono più leggibili e vanno riscritte:
            </p>
        ...
        <SettingsGroup destructive className="text-sm">
          ...
          <p className="text-muted-foreground">
            ...
            <span className="font-mono">
              {panel.lastRestoreFileName ?? "-"}
            </span>
          </p>
          {panel.lastRestoreError ? (
            <SettingsErrorNote
              label="Ultimo errore"
              message={panel.lastRestoreError}
            />
          ) : null}
        </SettingsGroup>
      </>
