...
export type PendingRestore =
  { type: "existing"; fileName: string } | { type: "upload"; file: File }
...
export const useBackupPanel = () => {
  ...
  const [savedValues, setSavedValues] =
    useState<BackupSettingsInput>(defaultForm)
  ...
  const [lastRestoreStatus, setLastRestoreStatus] =
    useState<SettingsRunStatus>("idle")
  const [lastRestoreError, setLastRestoreError] = useState<string | null>(null)
  const [lastRestoreFileName, setLastRestoreFileName] = useState<string | null>(
    null
  )
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      toast.error(
        getApiErrorMessage(error, "Impossibile caricare le impostazioni backup")
      )
    } finally {
      setIsLoading(false)
    }
  ...
  const loadDumpFiles = async () => {
    ...
    try {
      ...
      toast.error(
        getApiErrorMessage(error, "Impossibile caricare l'elenco dei dump")
      )
    } finally {
  ...
  const handleSave = async () => {
    ...
    if (
      !Number.isInteger(formValues.frequencyDays) ||
      formValues.frequencyDays <= 0
    ) {
    ...
    if (
      !Number.isInteger(formValues.maxBackupsToKeep) ||
      formValues.maxBackupsToKeep <= 0
    ) {
      toast.error(
        "Il numero di backup da mantenere deve essere un numero intero positivo"
      )
      return
    }
    ...
    if (formValues.smbEnabled) {
      if (
        !formValues.smbHost.trim() ||
        !formValues.smbShare.trim() ||
        !formValues.smbUsername.trim()
      ) {
      ...
      if (
        !Number.isInteger(formValues.smbPort) ||
        formValues.smbPort <= 0 ||
        formValues.smbPort > 65535
      ) {
    ...
    try {
      ...
      toast.error(
        getApiErrorMessage(error, "Impossibile salvare le impostazioni backup")
      )
    } finally {
      setIsSaving(false)
    }
  ...
  const handleTestSmbConnection = async () => {
    ...
    if (
      ...
      toast.error(
        "Per testare la connessione specifica almeno host, condivisione e utente"
      )
      return
    }
  ...
  const totalDumpsSize = dumpFiles.reduce(
    (total, dump) => total + dump.sizeBytes,
    0
  )
