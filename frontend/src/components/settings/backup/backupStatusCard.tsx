      ...
      <>
        <SettingsTileGrid>
          ...
          <SettingsTile
            ...
            value={
              panel.nextRunAt
                ? formatDateTime(panel.nextRunAt)
        ...
        {panel.lastDumpPath ? (
          <p className="text-xs text-muted-foreground">
            Percorso ultimo dump:{" "}
            <span className="font-mono">{panel.lastDumpPath}</span>
          </p>
        ) : null}

        {panel.lastError ? (
          <SettingsErrorNote
            label="Ultimo errore backup"
            message={panel.lastError}
          />
        ) : null}
        {panel.smbLastError ? (
          <SettingsErrorNote
            label="Ultimo errore NAS"
            message={panel.smbLastError}
          />
        ) : null}
      </>
    )}
